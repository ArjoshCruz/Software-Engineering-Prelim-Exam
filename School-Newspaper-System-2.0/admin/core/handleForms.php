...
if (isset($_POST['insertAdminArticleBtn'])) {
    ...
    // pass category_id to your article function
...
if (isset($_POST['editArticleBtn'])) {
	...
	if (isset($_FILES['article_image']) && $_FILES['article_image']['error'] === 0) {
		...
		if (in_array($fileType, $allowedTypes)) {
			...
			if (move_uploaded_file($_FILES['article_image']['tmp_name'], $targetFile)) {
				$imagePath = "uploads/" . $fileName; // relative path saved in DB
			}
		}
	}
...
if (isset($_POST['updateArticleVisibility'])) {
    ...
    if ($article) {
        ...
        if ($status == 1) {
            $message = "Your article titled '{$title}' has been accepted by an admin.";
        } else {
            $message = "Your article titled '{$title}' has been rejected by an admin.";
        }

        $notificationObj->sendNotification($author_id, $message);
    }

    header("Location: ../articles_submitted.php?statusUpdated=1");
    exit;
}
...
if (isset($_POST['acceptEditBtn']) || isset($_POST['rejectEditBtn'])) {
    ...
    if ($articleObj->respondToRequest($request_id, $author_id, $response)) {
