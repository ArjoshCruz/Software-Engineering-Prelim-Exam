...
class User extends Database {
     ...
     */
    public function registerAdmin($username, $email, $password, $contact_number) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO fiverr_clone_admin (username, email, password, contact_number) VALUES (?, ?, ?,  ?)";
        try {
            $this->executeNonQuery($sql, [$username, $email, $hashed_password, $contact_number]);
            return true;
    ...
    public function loginAdmin($email, $password) {
        $sql = "SELECT user_id, username, password FROM fiverr_clone_admin WHERE email = ?";
