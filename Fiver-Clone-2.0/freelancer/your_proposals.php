        ...
        <div class="col-md-6">
          ...
          <div class="card proposalCard shadow mt-4 mb-4">
            <div class="card-body">
              ...
              <p class="mt-2 h3"><?php echo $proposal['description']; ?></p>
              ...
              <form action="core/handleForms.php" method="POST">
                ...
                <form action="core/handleForms.php" method="POST" class="updateProposalForm d-none">
                  <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <label for="category_id_<?php echo $proposal['proposal_id']; ?>">Category</label>
                        <select id="category_id_<?php echo $proposal['proposal_id']; ?>" name="category_id" class="form-control category-dropdown" required>
                          <option value="">-- Select Category --</option>
                          <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['category_id']; ?>" <?= $cat['category_id'] == $proposal['category_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <label for="subcategory_id_<?php echo $proposal['proposal_id']; ?>">Subcategory</label>
                        <select id="subcategory_id_<?php echo $proposal['proposal_id']; ?>" name="subcategory_id" class="form-control subcategory-dropdown" required>
                          <option value="">-- Select Subcategory --</option>
                          <?php 
                            $subcats = $allSubcategories[$proposal['category_id']] ?? [];
                            foreach ($subcats as $sc): 
                          ?>
                            <option value="<?= $sc['subcategory_id']; ?>" <?= $sc['subcategory_id'] == $proposal['subcategory_id'] ? 'selected' : '' ?>>
    ...
    <script>
       $('.proposalCard').on('dblclick', function (event) {
        ...
        $('.category-dropdown').on('change', function() {
          ...
          const proposalId = $(this).attr('id').split('_')[2];
          const $sub = $('#subcategory_id_' + proposalId);
          $sub.html('<option value="">-- Select Subcategory --</option>');
          if (allSubcategories[catId]) {
            allSubcategories[catId].forEach(function(sc) {
              $sub.append(
